...
public class MeshGenerator : MonoBehaviour {
    ...
    public LayerMask fishAvoidLayer;
    ...
    void CreateChunkHolder()
    {
        if (chunkHolder == null)
        {
            // Look only for a holder under *this* GameObject to prevent cross-generator conflicts
            Transform existing = transform.Find(chunkHolderName);

            if (existing != null)
            {
                chunkHolder = existing.gameObject;
            }
            else
            {
                chunkHolder = new GameObject(chunkHolderName);
                chunkHolder.transform.parent = transform; // Parent to this generator
